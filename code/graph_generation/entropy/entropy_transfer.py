...
from code.graph_generation.entropy.ste import _get_symbol_sequence, symbolic_TE
...
def get_the_biggest_smallest_entropy(df):
    ...
    rect_vhigh = patches.Rectangle((-1, 0), 35, 3, linewidth=1, edgecolor='blue', facecolor='blue', alpha=0.3)
    rect_high = patches.Rectangle((-1, 3), 35, 4, linewidth=1, edgecolor='green', facecolor='green', alpha=0.3)
    rect_medium = patches.Rectangle((-1, 7), 35, 3, linewidth=1, edgecolor='orange', facecolor='orange', alpha=0.3)
    rect_low = patches.Rectangle((-1, 10), 35, 35, linewidth=1, edgecolor='gray', facecolor='gray', alpha=0.3)
    axs[0, 0].add_patch(rect_vhigh)
    axs[0, 0].add_patch(rect_high)
    axs[0, 0].add_patch(rect_medium)
    axs[0, 0].add_patch(rect_low)
    axs[0, 0].text(-5, 1.5, "very high", size=10, alpha = 0.8, color='blue')
    axs[0, 0].text(-5, 5, "high", size=10, alpha = 0.8, color='green')
    axs[0, 0].text(-5, 8.5, "medium", size=10, alpha = 0.8, color='orange')
    axs[0, 0].text(-5, 12.5, "low", size=10, alpha = 0.8, color='gray')
    ...
    axs[0, 1].text(-5, 1.5, "very high", size=10, alpha = 0.8, color='blue')
    axs[0, 1].text(-5, 5, "high", size=10, alpha = 0.8, color='green')
    axs[0, 1].text(-5, 8.5, "medium", size=10, alpha = 0.8, color='orange')
    axs[0, 1].text(-5, 12.5, "low", size=10, alpha = 0.8, color='gray')
    ...
    plt.savefig("./diagrams/entropy/entropy_min_4_grups2.pdf", format="pdf")
    plt.show()
    # print(res)
    # 
...
def unique_permutations(arr):
    ...
    def backtrack(start):
        if start == len(arr):
            result.append(tuple(arr[:]))  # Append a copy of the current permutation
            return

        for i in range(start, len(arr)):
            if i == start or arr[i] != arr[start]:
                arr[start], arr[i] = arr[i], arr[start]
                backtrack(start + 1)
                arr[start], arr[i] = arr[i], arr[start]

    result = []
    arr.sort()  # Ensure repeated elements are adjacent for easy skipping
    backtrack(0)
...
def test_random(df):
    ...
    de = e1-e2
    print('halo', e1)
    # print(res['Transform_Position_Country1'].to_numpy())
    # all_possible_permutations =unique_permutations(res['Transform_Position_Country1'].to_numpy())
    e = []
    ...
    for i in range(0, 100):
        ...
        e_random_1 = symbolic_TE(random_per_1, random_per_2, 1, 3)        
        e_random_2 = symbolic_TE(random_per_2, random_per_1, 1, 3)
    ...
    n, bins, patches = ax.hist(e, 10, density=False)
    ax.axvline(de, color='red')
    ax.text(de + 0.01, 3, f"actual delta: {de: .3f} ", size=10, alpha = 0.8, color='red')
    ax.set_xlabel("Entropy values")
    ax.set_ylabel("Count")
    plt.savefig("./diagrams/entropy/entropy_histogram_real_test.pdf", format="pdf")
    plt.show()


def plot_test_random(df):
    df['Week_Number_Year']  = df['Week_Number'].astype(str) + "_" +df['Year'].astype(str)
    filepath = "correct_extended_countries_with_entropy_all.csv"
    entropy_df = pd.read_csv(filepath,encoding='latin-1')
    entropy_df['Entropy'] = pd.to_numeric(entropy_df['Entropy'], errors='coerce')
    ...
    for i, c1 in enumerate(countries):
        for j, c2 in enumerate(countries):
            if i == j :
                continue
            songs = entropy_df[(entropy_df['Country1'] == c1) & (entropy_df['Country2'] == c2)]['Song']
            if len(songs) == 0:
                continue
            l = 0
            while l < 3:
            ...
            axs[i,j].hist(e, bins=10)
            axs[i,j].axvline(de, color="red")
            axs[i,j].text(de + 0.01, 50, f"actual delta: {de: .3f} ", size=10, alpha = 0.8, color='red')

            axs[i,j].set_title(f"{c1}-{c2}")
            axs[i,j].set_ylabel('Count')
            axs[i,j].set_xlabel('Entropy values')
    
    fig.tight_layout()
    plt.savefig("./diagrams/entropy/entropy_histogram_summary.pdf", format="pdf")
    # plt.show()

# get_the_biggest_smallest_entropy(df)
plot_test_random(df)
# example()
