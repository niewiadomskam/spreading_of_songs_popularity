...
def _get_symbol_sequence(x,l,m, vectorized_lookup):
    ...
    subsamples = Y.T
    string_values = np.array([np.array2string(np.array(xx), precision=1, separator=',',
                      suppress_small=True) for xx in subsamples])
    # print(string_values)
    # print(vectorized_lookup)
    return np.vstack(vectorized_lookup(string_values))
...
def symbolic_TE(ts1,ts2,l,m,vectorized_lookup):
    """
    ...
    x_sym = _get_symbol_sequence(ts1,l,m, vectorized_lookup)
    y_sym = _get_symbol_sequence(ts2,l,m, vectorized_lookup)
    # print(x_sym)
    # print(y_sym)

    hashmult = np.power(m, np.arange(m))

    # print(x_sym, hashmult)
